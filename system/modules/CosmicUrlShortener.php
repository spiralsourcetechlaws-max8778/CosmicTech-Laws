<?php
/**
 * COSMIC URL SHORTENER – INDUSTRIAL‑GRADE v2.0
 * Features:
 *   - Full REST API
 *   - QR code generation
...
class CosmicUrlShortener {
    ...
    /**
     * Redirect a short code to its target.
     */
    public function redirect($code, $password = null) {
        // Rate limiting
        $ip = $_SERVER['REMOTE_ADDR'] ?? '';
        $date = date('Y-m-d');
        $this->checkRateLimit($ip, $date);
    ...
    private function checkRateLimit($ip, $date) {
        if ($this->config['rate_limit'] <= 0) return;

        $stmt = $this->db->prepare("SELECT count FROM rate_limits WHERE ip = :ip AND date = :date");
        $stmt->bindValue(':ip', $ip, SQLITE3_TEXT);
        ...
        // Increment or insert
        if ($row) {
            $this->db->exec("UPDATE rate_limits SET count = count + 1 WHERE ip = '$ip' AND date = '$date'");
        } else {
            $stmt = $this->db->prepare("INSERT INTO rate_limits (ip, date, count) VALUES (:ip, :date, 1)");
            $stmt->bindValue(':ip', $ip, SQLITE3_TEXT);
            $stmt->bindValue(':date', $date, SQLITE3_TEXT);
            $stmt->execute();
        }
    ...
    private function getGeoIP($ip) {
        ...
        // Check cache
        $stmt = $this->db->prepare("SELECT * FROM geoip_cache WHERE ip = :ip AND cached_at > :time");
        $stmt->bindValue(':ip', $ip, SQLITE3_TEXT);
        $stmt->bindValue(':time', time() - 86400, SQLITE3_INTEGER); // cache for 24h
        $res = $stmt->execute();
        $cached = $res->fetchArray(SQLITE3_ASSOC);
        if ($cached) {
            return $cached;
        }
        ...
        try {
            ...
            // Save to cache
            $stmt = $this->db->prepare("
                INSERT OR REPLACE INTO geoip_cache (ip, country, city, latitude, longitude, asn, cached_at)
                VALUES (:ip, :country, :city, :lat, :lon, :asn, :time)
            ");
            $stmt->bindValue(':ip', $ip, SQLITE3_TEXT);
            $stmt->bindValue(':country', $data['country'], SQLITE3_TEXT);
            $stmt->bindValue(':city', $data['city'], SQLITE3_TEXT);
            $stmt->bindValue(':lat', $data['latitude'], SQLITE3_FLOAT);
            $stmt->bindValue(':lon', $data['longitude'], SQLITE3_FLOAT);
            $stmt->bindValue(':asn', $data['asn'], SQLITE3_TEXT);
            $stmt->bindValue(':time', time(), SQLITE3_INTEGER);
            $stmt->execute();
            return $data;
        } catch (Exception $e) {
    ...
    public function exportStats($linkId, $format = 'csv') {
        ...
        if ($format === 'csv') {
            ...
            fputcsv($out, ['Time', 'IP', 'Country', 'City', 'User Agent', 'Referer']);
            foreach ($clicks as $c) {
                fputcsv($out, [
